...
int stream_vprintf(struct write_stream * stream, const char *format, va_list args) {
    ...
    while (*fmt) {
        ...
        case_int:
        {
            ...
            if (qualifier == 'I')
            {
                if (flags & FL_SIGNED)
                {
                    ...
                    number64 = va_arg(args, uint64_t);
            }
            else if (qualifier == 'l' || qualifier == 'L')
            {
                if (flags & FL_SIGNED)
                {
                    long n = va_arg(args, int32_t);
                    if (n < 0)
                    {
            ...
            else if (qualifier == 'h')
            {
                if (flags & FL_SIGNED)
                {
                    short n = va_arg(args, /*int16_t*/int);
                    if (n < 0)
                    {
                        number = -n;
                        flags |= FL_NEGATIVE;
                    ...
                    number = va_arg(args, /*uint16_t*/unsigned int);
            }
            ...
            textlen = (int)(&buffer[MAXBUFFER - 1] - text); // Compute length of number
        ...
        case 'g':
            ...
            textlen = (int)strlen(text); // Compute length of text
            break;
        ...
        if (!(flags & (FL_LEFT | FL_LEADZERO))) {
            if (padding > 0) {
                int n = stream_putc_count(stream, ' ', padding);
                if (n < 0) goto PRINT_COMPLETE;
                
                cnt += n;
                padding -= n;
                if (padding > 0) goto PRINT_COMPLETE;
            }
        }

        // Write prefix
        if (prefixlen > 0) {
            int n = stream_write(stream, prefix, prefixlen);
            if (n < 0) goto PRINT_COMPLETE;
            assert(n <= prefixlen);
            cnt += n;
            if (n < prefixlen) goto PRINT_COMPLETE;
        }
        ...
        if ((flags & FL_LEADZERO) && !(flags & FL_LEFT)) {
            // Write leading zeros
            if (padding > 0) {
                int n = stream_putc_count(stream, '0', padding);
                if (n < 0) goto PRINT_COMPLETE;
                cnt+=n;
                padding-=n;
                if (padding > 0) goto PRINT_COMPLETE;
        ...
        // Put out text
        if (textlen > 0) {
            int n = stream_write(stream, text, textlen);
            if (n < 0) goto PRINT_COMPLETE;
            ...
            if (textlen > 0) goto PRINT_COMPLETE;
        }
