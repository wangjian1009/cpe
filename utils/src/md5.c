...
/* forward declaration */
static void cpe_md5_transform (uint32_t *buf, uint32_t *in);
...
void cpe_md5_ctx_init(cpe_md5_ctx_t mdContext) {
...
void cpe_md5_ctx_update(cpe_md5_ctx_t mdContext, void const * _inBuf, size_t inLen) {
    uint32_t in[16];
    int mdi;
    unsigned int i, ii;
    uint8_t const * inBuf = _inBuf;
    ...
    while (inLen--) {
        ...
        if (mdi == 0x40) {
            ...
            cpe_md5_transform (mdContext->buf, in);
            mdi = 0;
        }
    }
}

void cpe_md5_ctx_final(cpe_md5_ctx_t mdContext) {
    uint32_t in[16];
    int mdi;
    unsigned int i, ii;
    unsigned int padLen;
    ...
    cpe_md5_ctx_update(mdContext, PADDING, padLen);
    ...
    for (i = 0, ii = 0; i < 4; i++, ii += 4) {
        mdContext->value.digest[ii] = (unsigned char)(mdContext->buf[i] & 0xFF);
        mdContext->value.digest[ii+1] =
            (unsigned char)((mdContext->buf[i] >> 8) & 0xFF);
        mdContext->value.digest[ii+2] =
            (unsigned char)((mdContext->buf[i] >> 16) & 0xFF);
        mdContext->value.digest[ii+3] =
            (unsigned char)((mdContext->buf[i] >> 24) & 0xFF);
    }
}

/* Basic MD5 step. cpe_md5_transform buf based on in.
 */
static void cpe_md5_transform (uint32_t *buf, uint32_t *in) {
...
void cpe_md5_print(write_stream_t s, cpe_md5_value_t value) {
    int i;
    for (i = 0; i < 16; i++) {
        stream_printf(s, "%02x", value->digest[i]);
    }
}
...
const char * cpe_md5_dump(cpe_md5_value_t value, mem_buffer_t buffer) {
    struct write_stream_buffer stream = CPE_WRITE_STREAM_BUFFER_INITIALIZER(buffer);

    mem_buffer_clear_data(buffer);

    cpe_md5_print((write_stream_t)&stream, value);
