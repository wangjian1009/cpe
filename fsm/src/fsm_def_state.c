...
fsm_def_state_t fsm_def_state_find_by_name(fsm_def_machine_t m, const char * name) {
    ...
    cpe_hs_init((cpe_hash_string_t)(void*)name_buf, sizeof(name_buf), name);
    key.m_name = (cpe_hash_string_t)(void*)name_buf;
...
uint32_t fsm_def_state_hash(fsm_def_state_t data, void * ctx) {
    return cpe_hs_value(data->m_name);
}

int fsm_def_state_eq(const fsm_def_state_t l, const fsm_def_state_t r, void * ctx) {
